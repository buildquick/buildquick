// Adapted from https://gist.github.com/barretts/e90d7e5251f36b183c67e02ba54c9ae1
...
class Color {
  ...
  clamp(value: number, min = 0, max = 255) {
    if (value > max) {
      value = max;
    } else if (value < min) {
      value = min;
...
class Solver {
  ...
  solveWide() {
    ...
    for (let i = 0; best.loss > 25 && i < 3; i++) {
      const result = this.spsa(A, a, c, best.values, 1000);
  ...
  spsa(A: number, a: number[], c: number, values: number[], iters: number) {
    ...
    let best = [50, 20, 3750, 50, 100, 100];
  ...
  css(filters: number[]) {
    ...
    const filter = `invert(${fmt(0)}%) sepia(${fmt(1)}%) saturate(${fmt(
      2
    )}%) hue-rotate(${fmt(3, 3.6)}deg) brightness(${fmt(4)}%) contrast(${fmt(
      5
    )}%)`;

    if (this.target.a) return `${filter} opacity(${this.target.a})`;
    return filter;
  }
}
